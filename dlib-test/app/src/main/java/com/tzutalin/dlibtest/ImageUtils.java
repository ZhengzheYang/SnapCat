...
public class ImageUtils {
     ...
     * @param bitmap The bitmap to save.
     */
    public static void saveBitmapAsJPG(final Bitmap bitmap) {
        final String root =
                Environment.getExternalStorageDirectory() + File.separator + "dlib";
        Timber.tag(TAG).d(String.format("Saving %dx%d bitmap to %s.", bitmap.getWidth(), bitmap.getHeight(), root));
        final File myDir = new File(root);
        if (!myDir.mkdirs()) {
            Timber.tag(TAG).e("Make dir failed");
        ...
        final String fname = new SimpleDateFormat("yyyy-MM-dd hh-mm-ss'.tsv'").format(new Date());
        final File file = new File(myDir, fname);
        if (file.exists()) {
            file.delete();
        }
